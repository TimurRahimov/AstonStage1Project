...
public class ExtraTimSort {
    ...
    public static <T extends Comparable<T>> void extraSort(List<T> list, Predicate<T> predicate) {
        if (predicate == null) {
            sort(list);
        }
        ...
        ListIterator<T> tmpIter = tmp.listIterator();

        for(int i = 0; i < list.size(); i++) {
            if (list.get(i) == null && tmpIter.hasNext()) {
                list.set(i, tmpIter.next());
            }
        }
    }
}
